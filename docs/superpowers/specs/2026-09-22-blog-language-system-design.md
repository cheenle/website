...
- `ba-tldr` 存在：论说文 ≥4 条、实操文 ≥3 条；每条有 `data-claim-type ∈ {fact, inference, thesis}`
...
- `——` ≤6‰
- 中文正文无半角直引号（排除 `<code>` / `<pre>` 内的代码——如 `"FT4222 A"` 这类设备描述符字符串是合法的）
...
**豁免机制**：批次 0–1 落地时，超标项进入带到期批次号的"待清"白名单；**批次 7 清空**，验收时不得残留。
