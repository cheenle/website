...
- **易占** (`yijing/`) — I Ching divination app（易经占卜）. Deployed to `https://www.vlsc.net/yijing/`. Pure static HTML/CSS/JS, Chinese-only UI, 水墨宣纸 theme in `css/yijing.css` (cinnabar `#9e2b25` accent). Data lives in `js/data/hexagrams.js` (64 hexagrams: received text of 卦辞/彖/象/爻辞/小象 + vernacular readings + per-topic advice) and `js/data/trigrams.js`; `check_hexagrams.py` is the pre-deploy data gate (run `ALLOW_PARTIAL=1` while authoring batches). Logic: `cast.js` (three-coin casting), `gua.js` (本卦/之卦/互卦 + Zhu Xi 断法 in `duanCi`), `app.js` (DOM/state). Features beyond the classical reading: multi-turn follow-up (`/yijing/api/chat`, history `messages` array with the hexagram context as first message), per-category dynamic system prompts, 应验跟踪 (track period + revisit modal, outcomes bound to the original cast and AI reply in `feedback.jsonl`), native-Canvas share poster, WebAudio coin sound + DeviceMotion shake-to-cast, a 黄帝内经 grounding pool for 健康-category readings/chats (`NEIJING_PASSAGES`, citations restricted to the pool with 篇名) plus an 易卦×内经 cross-reference layer (`health_context_lines`: 说卦传 body mapping, 爻位配身, 当令四时五脏, 生克推演) driving a five-section deep health reading, and a 梅花易数 engine (`js/meihua.js`: 时间/数字起卦, 体用生克) whose readings use a separate meihua system prompt. The 「AI 进一步解读」 button posts to `/yijing/api/interpret`, proxied by nginx (`location ^~ /yijing/api/`) to `server/llm_proxy.py` — a stdlib service on 127.0.0.1:8100 (systemd unit `yijing-llm.service`, `EnvironmentFile=/etc/yijing/llm.env`, token never in repo or browser) which calls the Anthropic-compatible DashScope endpoint. Improvement loop: interpret/feedback JSONL logs in `/home/cheenle/yijing-llm-logs/`, golden-set eval `server/eval_run.py` driven weekly by `yijing-llm-eval.timer` (see `server/ITERATION.md`); bump `PROMPT_VERSION` with every prompt change. Reinstall after deploy: `install -m 644 /var/www/vlsc.net/yijing/server/yijing-llm.service /etc/systemd/system/ && systemctl daemon-reload && systemctl restart yijing-llm`. Node tests in `yijing/tests/` (`cd yijing && node --test`), excluded from the deploy tarball along with `check_hexagrams.py` and `deploy.sh`.
