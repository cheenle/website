...
| section id | 必须包含 |
|---|---|
| `ledger-shrank` | **开场即给新事实**：`7,007,437,567` → `6,774,077,763`（−`349,611,847`）；机制 = claude-code 保留期（`~/.claude/.last-cleanup` = `2026-09-07T13:13:57Z`）；对照 = codex `547,796,667` **逐位不变**；`pi` 反而 +；结论 = **证据必须被快照，不能被累计**。标 `fact` + `thesis` |
...
| `window` | claude-code 清理证据；codex 冻结；各工具保留语义差异；"总额不是单调量" |
| `repos` | 附录 A4 的 HEAD 口径表 + `--all` 口径警告 |
| `boundaries` | 八条测量边界（沿用原附录 C）扩充：新增窗口口径、venv 方法变更、`--all` 口径警告 |
