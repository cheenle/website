...
for k in "${DEPLOY_KEYS[@]}"; do
  for s in "${SITES[@]}"; do
    ...
    [ "$key" = "$k" ] || continue
    if [ ! -d "$ldir" ]; then
      echo -e "  ${RED}✗ $key: local dir missing: $ldir${NC}"; exit 1
    fi
    ok=1
    for f in index.html css/octen.css; do
      [ -f "$ldir/$f" ] || { echo -e "  ${RED}✗ $key: missing $f${NC}"; ok=0; }
    done
    [ -f "$ldir/js/global-nav.js" ] || { echo -e "  ${RED}✗ $key: missing js/global-nav.js${NC}"; ok=0; }
